...
- **WebDAV**: deployment may provide an endpoint default; every VOS user saves their own account and password in Settings → Assets & Transcription → Remote media → WebDAV, then uses Test Connection to verify that the root is reachable.
- **AI Album**: inside the same VOS, V-ChatCut first tries the current user's OIDC token for `com.ictrek.ai-album`; if the AI Album API returns 401, or for standalone Immich, each VOS user saves their own API key in Settings → Assets & Transcription → Remote media → AI Album, then uses Test Connection to verify authentication.
