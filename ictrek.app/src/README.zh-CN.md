...
- **WebDAV**：管理员可提供地址默认值；每个 VOS 用户在 V-ChatCut 设置中填写自己的用户名和密码。
- **AI 相册**：同一 VOS 内默认通过当前登录用户的 OIDC token 访问 `com.ictrek.ai-album`；独立 Immich 或需要兜底时，每个 VOS 用户可在 V-ChatCut 设置中填写自己的 API Key。
