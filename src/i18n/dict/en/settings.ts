...
export default {
  ...
  'V-Router · VOS 账户模型': 'V-Router · VOS account models',
  'V-Router（VOS 账户）': 'V-Router (VOS account)',
  '从 V-Router 选择': 'Select from V-Router',
  '使用当前 VOS 登录账户读取 V-Router 中可见的 public 模型和本账户 private 模型；无需在 V-ChatCut 保存 API Key 或上游地址。': 'Use the current VOS account to load visible public models and this account\'s private models from V-Router. V-ChatCut does not store an API key or upstream URL.',
  '使用当前 VOS 登录账户鉴权；模型及上游凭据只在 V-Router 中配置。': 'Authenticate with the current VOS account. Configure models and upstream credentials only in V-Router.',
  '使用当前 VOS 登录账户鉴权；模型及上游凭据只在 V-Router 中配置。当前接入文生视频，参数能力由所选上游模型决定。': 'Authenticate with the current VOS account. Configure models and upstream credentials only in V-Router. This integration currently supports text-to-video; available parameters depend on the selected upstream model.',
  '使用当前 VOS 登录账户鉴权；测试连接后选择声明 audio/transcriptions 的模型。': 'Authenticate with the current VOS account, then test the connection and select a model that declares audio/transcriptions.',
  '使用当前 VOS 登录账户鉴权；测试连接后选择声明 audio/speech 的模型。': 'Authenticate with the current VOS account, then test the connection and select a model that declares audio/speech.',
  '测试连接后选择声明 images/generations 的模型。': 'Test the connection, then select a model that declares images/generations.',
  '测试连接后选择声明 audio/speech 的模型。': 'Test the connection, then select a model that declares audio/speech.',
  '测试连接后选择声明 videos/generations 的模型。': 'Test the connection, then select a model that declares videos/generations.',
  '点击测试连接后选择 V-Router 返回的模型。': 'Test the connection, then select a model returned by V-Router.',
  ...
  '测试并读取模型与音色': 'Test & load models and voices',
  '选择音色': 'Choose voice',
  '默认音色': 'Default voice',
  '先选择模型': 'Choose a model first',
  '选好模型后再次测试，即可读取该模型支持的音色。': 'Choose a model, then test again to load its supported voices.',
